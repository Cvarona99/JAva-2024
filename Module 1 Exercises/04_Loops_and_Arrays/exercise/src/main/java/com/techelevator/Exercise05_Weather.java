...
public class Exercise05_Weather {
    ...
    */
    public int belowFreezing(int[] dailyHighs) {
        ...
        while (index < dailyHighs.length)
        {
            if (dailyHighs[index] <= FREEZING_TEMPERATURE)
            {
                freezingDays++;
                index++;
    ...
    */
    public int hottestDay(int[] dailyHighs) {
        ...
        while (index < dailyHighs.length)
        {
            ...
            if (hottestDay < dailyHighs[index])
            {
                hottestDay = dailyHighs[index];
                index++;

                if (index == dailyHighs.length-1)
                {
                    if (hottestDay < dailyHighs[index])
                    {
                        hottestDay = dailyHighs[index];
