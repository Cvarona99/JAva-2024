...
public class Exercise04_Cards {
    ...
    */
    ...
    public String[] discardFirstCard(String[] hand) {
        ...
        String[] drawCard = new String[hand.length-1];
        int size = hand.length;
        int target = 0;


        for ( int i = target; i < hand.length; i++) {
            //   System.out.println(remainingDeck[i]);
            if (i == hand.length-1)  {
                break;
            }
    ...
    public String[] discardTopCard(String[] remainingDeck) {
        ...
        String[] drawCard = new String[remainingDeck.length-1];
        int size = remainingDeck.length;
        int target = 0;


        for ( int i = target; i < remainingDeck.length; i++) {
         //   System.out.println(remainingDeck[i]);
            if (i == remainingDeck.length-1)  {
                break;
            }

            drawCard[i] = remainingDeck[i+1];
          //  System.out.println(drawCard[i]);
        }
