...
public class Exercise02_BoardingGate {
    ...
    public boolean[] generateSeatingChart(int numberOfSeats) {
        ...
        int listSize = numberOfSeats;
        boolean[] seats = new boolean[listSize];
    ...
    public int getAvailableSeatCount(boolean[] seatingChart) {
        ...
        while (index < seatingChart.length) {
            if (seatingChart[index] == AVAILABLE) {
                availableSeats++;
            }
            index++;
        }
    ...
    public int getNumberOfFullRows(boolean[] seatingChart) {
        ...
        while (index < seatingChart.length) {
            if (seatingChart[index] == OCCUPIED) {
                index++;
                if (seatingChart[index] == OCCUPIED) {
                    index++;
                    if (seatingChart[index] == OCCUPIED) {
            ...
            else if (seatingChart[index] == AVAILABLE) {
              index += 3;
            }
