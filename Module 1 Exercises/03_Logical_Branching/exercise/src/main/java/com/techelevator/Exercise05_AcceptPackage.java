...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        if (weightPounds <= MAX_WEIGHT_POUNDS && (lengthInches * widthInches * heightInches <= MAX_CUBIC_INCHES) && isSurchargePaid)
        {
            return true;
        }
        else
            return weightPounds <= MAX_WEIGHT_POUNDS && (lengthInches * widthInches * heightInches <= MAX_CUBIC_INCHES) && lengthInches <= MAX_DIMENSION_INCHES && widthInches <= MAX_DIMENSION_INCHES && heightInches <= MAX_DIMENSION_INCHES;
